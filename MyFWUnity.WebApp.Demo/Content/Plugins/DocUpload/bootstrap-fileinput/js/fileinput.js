...
(function (factory) {
    ...
    $h = {
        ...
        STYLE_SETTING: 'style="width:{width};height:{height};max-width:80px;"',
    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            tActions = '{drag}\n' +
                ...
                '</div>';
            //noinspection HtmlUnknownAttribute
            tActionDelete = '<a  class="kv-file-remove {removeClass}" href="javascript:;" style="color:#fff;text-decoration:none;" ' +
                'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}删除</a>\n';
            ...
            tTagBef = '<div class="file-preview-frame {frameClass}"  onmouseover="frameover(this)" data-filename="{filename}" onmouseout="frameout(this)"  id="{previewId}" data-fileindex="{fileindex}"' +
            ...
            tFlash = '<object class="kv-preview-data file-object" type="application/x-shockwave-flash" ' +
                ...
                '\n</object>\n';
            tObject = '<div class="kv-preview-data file-object {typeCss}" data="{data}" type="{type}" ' +
            ...
            self.defaults = {
                ...
                fileActionSettings: {
                    ...
                    showZoom: true,
                    showDrag: true,
                    removeIcon: '<i class="glyphicon glyphicon-trash"  style="color:#fff"></i>',
                    removeClass: '',
                    removeTitle: 'Remove file',
                    uploadIcon: '<i class="glyphicon glyphicon-upload text-info"></i>',
                    uploadClass: 'btn-xs btn-default',
                    uploadTitle: 'Upload file',
                    zoomIcon: '<i class="glyphicon glyphicon-zoom-in"></i>',
                    zoomClass: 'btn-xs btn-default',
        ...
        _listen: function () {
            ...
            self._handler($cont.find('.fileinput-remove'), 'click', $.proxy(self.clear, self));
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            formdata.append('guid', currentNode);
            self._ajaxSubmit(fnBefore, fnSuccess, fnComplete, fnError, previewId, i);
        },
        _uploadSingle1: function (i, files, allFiles, fileName, islast, allfile, guid) {
            var self = this, total = self.getFileStack().length, formdata = new FormData(), outData,
                previewId = self.previewInitId + "-" + i, $thumb, chkComplete, $btnUpload, $btnDelete,
                hasPostData = self.filestack.length > 0 || !$.isEmptyObject(self.uploadExtraData),
                $prog = $('#' + previewId).find('.file-thumb-progress'),
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                $.extend(true, params, outData);
                setTimeout(function () {
                    if ($h.isEmpty(data) || $h.isEmpty(data.ResultData.message)) {
                        if (self.showPreview) {
                            self._setThumbStatus($thumb, 'Success');
                            $btnUpload.hide();
                            self._initUploadSuccess(data, $thumb, allFiles);
                        ...
                        self._showUploadError(data.ResultData.message, params);
                        self._setPreviewError($thumb, i);
                        if (allFiles) {
                            updateUploadLog(i, pid);
                        }
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
                var op = self.ajaxOperations.uploadThumb,
                    errMsg = self._parseError(op, jqXHR, errorThrown, (allFiles ? files.name : null));
                setTimeout(function () {
                    if (allFiles) {
                        updateUploadLog(i, previewId);
                    }
            ...
            formdata.append(self.uploadFileAttr, files.file, files.name);
            formdata.append('file_id', i);
            formdata.append('guid', guid);
            formdata.append('size', files.file.size);
            formdata.append('fileName', fileName);
            if (DocUploadFile.extendupload.currentfolder != undefined && DocUploadFile.extendupload.currentfolder != null) {
                formdata.append('currentNode', DocUploadFile.extendupload.currentfolder);
            }
            formdata.append('part', files.part);
            var userids = [];
            DocUploadFile.extendupload.formdata.forEach(function (v, k) {
                formdata.append(k, v);
            })
            if (islast) {
                formdata.append('isLast', true);
                formdata.append('allfilesCount', allfile.length);
            }
            self._ajaxSubmit(fnBefore, fnSuccess, fnComplete, fnError, previewId, i);
        },
            ...
            $preview.find($h.FRAMES + ' .kv-file-remove').each(function () {
                ...
                self._handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        $h.cleanZoomCache($preview.find('#zoom-' + id));
                        var filename = $frame.data("filename");
                        self.updateStack(ind, undefined, filename);
        ...
        _previewDefault: function (file, previewId, isDisabled) {
            ...
            $preview.append("\n" + content);
            // $(".upload-doc-left").css("width", "500px");
           // $(".upload-doc-right").show();
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if (i >= numFiles) {
                    ...
                    $(".fileinput-remove-button").prop('disabled', '');
                    $status.html('');
                    return;
                }
                ...
                if ($preview.length && FileReader !== undefined) {
                    ...
                    $container.addClass('file-thumb-loading');
                    $(".fileinput-remove-button").prop('disabled', 'disabled');
        ...
        _setFileDropZoneTitle: function () {
            ...
            if ($zone.find($h.FRAMES).length === 0 && $h.isEmpty(self.defaultPreviewContent)) {
                var Html = '<div  class="' + self.dropZoneTitleClass + '"><img src="" /><p><p/><div>';
                Html += '<div class="upload-doc-index">';
                Html += '  <img src="/Content/Plugins/DocUpload/bootstrap-fileinput/img/doc-upload-icon.png" width="150" />';
                Html += '  <p>将文件拖拽至此</p>';
                Html += ' </div>';
                $zone.prepend(Html);
                // $(".upload-doc-left").css("width", "100%;");
                //$(".upload-doc-right").hide();
        ...
        _change: function (e) {
            ...
            if (isDragDrop) {
                $.each(files, function (i, f) {
                    if (f && !f.type && f.size !== undefined && f.size == 4096) {
                        folders++;
                    } else {
        ...
        updateStack: function (i, file, name) {
            var self = this;
            if (i >= self.filestack.length) {
                i = self.filestack.length - 1;
            }
            for (var j = 0; j < self.filestack.length; j++) {
                if (name != undefined) {
                    if (self.filestack[j].name == name) {
                        i = j;
                    }
                }
            }
            ...
            if (self.uploadAsync) {
                ...
                for (i = 0; i < len; i++) {
                    if (self.filestack[i] !== undefined) {
                        //self.uploadUrl = "../api/DocManager/UploadBigFile";
                        allfile = cutFile(self.filestack[i], 1024 * 1024 * 4);
                        var guid = new GUID();
                        guid = guid.newGUID();
                        for (var x = 0; x < allfile.length; x++) {
                            islast = x == allfile.length - 1 ? true : false;
                            self._uploadSingle1(i, allfile[x], true, self.filestack[i].name, islast, allfile, guid);
                        }
                    }
                }
                return;
