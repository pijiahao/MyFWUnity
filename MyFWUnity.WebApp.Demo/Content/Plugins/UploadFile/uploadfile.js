(function ($) {
    ...
    Uploadfile.prototype.clickfile = function (file) {
        ...
        if (file.files.length > 0) {
            ...
            var filesIndex = file.files.length;
            $.each(file.files, function (k, v) {
                ...
                reader.onload = function (theFile) {
                    ...
                    if (v.size <= self.default.maxLength) {
                        ...
                        i = Math.floor(Math.log(self.default.maxLength) / Math.log(1024));
                        sweetAlert("警告", "当前文件仅支持单个文件上传大小为" + (self.default.maxLength / Math.pow(1024, i)).toFixed(2) * 1 + ' ' + sizes[i] + ",超出文件大小的已过滤", "warning");
                    }
                    filesIndex--;
                    if (filesIndex == 0) {
                        //  self.upload();
    ...
    Uploadfile.prototype.getimghtml = function (filename, fileurl) {
        ...
        var ext = filearry[filearry.length - 1].toLowerCase();
        if (ext == "doc" || ext == "docx") {
            strhtml += '<img src="/Content/Plugins/UploadFile/images/word.png" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';

        }
        else if (ext == "xls" || ext == "xlsx") {
            strhtml += '<img src="/Content/Plugins/UploadFile/images/excel.png" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';

        }
        else if (ext == "ppt" || ext == "pptx") {
            strhtml += '<img src="/Content/Plugins/UploadFile/images/ppt.png" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';
        }
        else if (ext == "pdf") {
            strhtml += '<img src="/Content/Plugins/UploadFile/images/pdf.png" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';
        }
        else {

            strhtml += '<img src="/Content/Plugins/UploadFile/images/attachment.png" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';

            //var imgArray = ['jpg', 'gif', 'png'];
            //if ($.inArray(ext, imgArray) > -1) {
            ...
            //}
            //else {
            //    strhtml += '<img src="/Content/Plugins/UploadFile/images/notfile.jpg" class="file-preview-image kv-preview-data" onclick="ImgMagnifying(this)" title= "' + filename + '" alt= "' + filename + '" style= "width:auto;height:40px;max-width:80px;" >';
            //}
                    ...
                    this.$this.find("a.kv-file-remove").off("click").on("click", function () {
                        if (!self.disable) {
                            ...
                            if (self.default.isremoveloaddata) {
                                ...
                                self.default.deletefile(self.loaddata[index], this, self);
                            }
                        }
                    })
                }
